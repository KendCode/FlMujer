...
<div class="container mt-4">
    ...
    <form action="{{ route('casos.fichaAtencionEvaluacion.update', ['caso' => $ficha->caso->id, 'ficha' => $ficha->id]) }}" method="POST" class="card shadow p-4">
        @csrf
        @method('PUT')
        ...
        <div class="row">
            <div class="col-md-4 mb-3">
                ...
                <input type="date" name="fecha" class="form-control" value="{{ old('fecha', $ficha->fecha->format('Y-m-d')) }}" required>
            </div>
        ...
        <div class="mb-3">
            ...
            <input type="text" name="nombres_apellidos" class="form-control" value="{{ old('nombres_apellidos', $ficha->nombres_apellidos) }}" readonly>
        </div>
        ...
        <div class="border rounded p-3 mb-3">
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="radio" name="busco_ayuda" value="Si" {{ old('busco_ayuda', $ficha->busco_ayuda) == 'Si' ? 'checked' : '' }}> Sí
                    <input type="radio" name="busco_ayuda" value="No" class="ms-2" {{ old('busco_ayuda', $ficha->busco_ayuda) == 'No' ? 'checked' : '' }}> No
                    <input type="text" name="donde_busco_ayuda" class="form-control mt-2" placeholder="¿Dónde?" value="{{ old('donde_busco_ayuda', $ficha->donde_busco_ayuda) }}">
                </div>

                <div class="col-md-6 mb-3">
                    <label>¿Recibió apoyo psicológico?</label><br>
                    <input type="radio" name="recibio_apoyo" value="Si" {{ old('recibio_apoyo', $ficha->recibio_apoyo) == 'Si' ? 'checked' : '' }}> Sí
                    <input type="radio" name="recibio_apoyo" value="No" class="ms-2" {{ old('recibio_apoyo', $ficha->recibio_apoyo) == 'No' ? 'checked' : '' }}> No
                    <input type="text" name="donde_apoyo" class="form-control mt-2" placeholder="¿Dónde?" value="{{ old('donde_apoyo', $ficha->donde_apoyo) }}">
                </div>

                <div class="col-md-6 mb-3">
                    <label>¿Concluyó la terapia?</label><br>
                    <input type="radio" name="concluyo_terapia" value="Si" {{ old('concluyo_terapia', $ficha->concluyo_terapia) == 'Si' ? 'checked' : '' }}> Sí
                    <input type="radio" name="concluyo_terapia" value="No" class="ms-2" {{ old('concluyo_terapia', $ficha->concluyo_terapia) == 'No' ? 'checked' : '' }}> No
                    <input type="text" name="cuando_terapia" class="form-control mt-2" placeholder="¿Cuándo?" value="{{ old('cuando_terapia', $ficha->cuando_terapia) }}">
                </div>

                <div class="col-md-6 mb-3">
                    <label>Enfermedades en los últimos años:</label>
                    <input type="text" name="enfermedades" class="form-control" value="{{ old('enfermedades', $ficha->enfermedades) }}">
                </div>
            </div>
        </div>
        ...
        <div class="mb-3">
            ...
            @foreach (['Individual', 'Pareja', 'Familia'] as $estrategia)
                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="estrategia" value="{{ $estrategia }}" {{ old('estrategia', $ficha->estrategia) == $estrategia ? 'checked' : '' }}>
        ...
        <div class="row mb-3">
            <div class="col-md-6">
                ...
                <input type="date" name="fecha_proxima" class="form-control" value="{{ old('fecha_proxima', optional($ficha->fecha_proxima)->format('Y-m-d')) }}">
            </div>
            <div class="col-md-6">
                ...
                @foreach (['Legal', 'Trabajo Social', 'Espiritual', 'Médico'] as $remito)
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="checkbox" name="remito[]" value="{{ $remito }}" {{ in_array($remito, old('remito', $ficha->remito ?? [])) ? 'checked' : '' }}>
        ...
        <div class="text-center">
            ...
            <a href="{{ route('casos.fichaAtencionEvaluacion.index', $ficha->caso->id) }}" class="btn btn-secondary px-4 ms-2">Cancelar</a>
        </div>
    </form>
</div>
@endsection
